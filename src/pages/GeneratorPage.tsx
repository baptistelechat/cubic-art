...
export function GeneratorPage() {
  ...
  const { config, isProcessing, mosaicResult, setIsProcessing, setMosaicResult, updateConfig } = useStore();
  
  // État pour stocker le fichier actuel
  const [currentFile, setCurrentFile] = useState<File | null>(null);
  
  // Fonctions stabilisées avec useCallback pour éviter les boucles infinies
  const stableSetIsProcessing = useCallback((processing: boolean) => {
    setIsProcessing(processing);
  }, [setIsProcessing]);
  
  const stableSetMosaicResult = useCallback((result: MosaicResult) => {
    ...
    const regenerateMosaic = async () => {
      if (currentFile && !isProcessing) {
        try {
          ...
          setError('Erreur lors de la régénération de la mosaïque. Veuillez réessayer.');
          stableSetIsProcessing(false);
        }
      }
    };
